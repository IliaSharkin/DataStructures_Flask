...
def get_all_users_descending_id():
...
@app.route('/user/ascanding_id', methods=['GET'])
...
def create_blog_post(user_id):
    ...
    user = User.query.filter_by(id=user_id).first()
    if not user:
        return jsonify({"massge": "user does not exist!"})
    ...
    ht.add_key_value("user_id", data["title"])
...
def get_all_blog_posts(blog_post_id):
    ...
    bst = binaty_serch_tree.BinarySearchTree()
